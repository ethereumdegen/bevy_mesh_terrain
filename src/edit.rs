                  ...
                  EditingTool::SetSplatMap(r,g,b,radius,save_to_disk ) => {
                       ...
                       if let Some(splat_image_handle) = chunk_data.get_splat_texture_image(){
                         if let Some(img) = images.get_mut( splat_image_handle ){
                             ...
                             // Calculate the pixel position and radius in pixels
                                ...
                                if  img.texture_descriptor.format == TextureFormat::Rgba8Unorm || img.texture_descriptor.format == TextureFormat::Rgba8UnormSrgb  {
                    //                let img_data = img.data.as_mut_slice();
                                    ...
                                    /*img.texture_descriptor.size = Extent3d{
                                      ...
                                      chunk_data.set_splat_texture_image(updated_image_handle.clone()); //is this necessary? i think so in case the height is modified 
                                      
                                      
                                        if let Some(material_handle) = &chunk_data.material_handle{
                                            
                                            if let Some(terrain_material) = terrain_materials.get_mut(  material_handle  ){
                                                
                                                //this should let us avoid rebuilding the entire mesh 
                                                terrain_material.splat_texture = Some(updated_image_handle);
                                    ...
                                    println!("incorrect tex format {:?}", img.texture_descriptor.format);
                                }
